...
class TestConversationManagement:
    ...
    def test_update_conversation_title(self, test_db):
        ...
        conversation = create_conversation(db=db, user_id=user.id, title="Original Title")

        # Mettre à jour le titre
        updated = update_conversation_title(
            db=db, conversation_id=conversation.id, user_id=user.id, title="Updated Title"
        )
    ...
    def test_get_conversation_messages_empty(self, test_db):
        ...
        conversation = create_conversation(db=db, user_id=user.id, title="Empty Conversation")
    ...
    def test_get_conversation_with_wrong_user(self, test_db):
        ...
        db = test_db()
        user1 = create_user_db(db=db, name="User 1", email="user1@example.com", password="TestPassword123!")
        user2 = create_user_db(db=db, name="User 2", email="user2@example.com", password="TestPassword123!")

        conversation = create_conversation(db=db, user_id=user1.id, title="User 1 Conversation")
