...
def create_shared_conversation(
    ...
    existing = (
        ...
        .filter(SharedConversation.conversation_id == conversation_id, SharedConversation.is_active == True)
        .first()
    )
...
def get_shared_conversation(db: Session, share_token: str) -> Optional[Dict[str, Any]]:
    """
    ...
    shared = (
        db.query(SharedConversation)
        .filter(SharedConversation.share_token == share_token, SharedConversation.is_active == True)
        .first()
    )
...
def revoke_shared_conversation(db: Session, conversation_id: int, user_id: int) -> bool:
    """
...
def get_user_shared_conversations(db: Session, user_id: int) -> List[Dict[str, Any]]:
    """
    ...
    return [
        {
            ...
            "conversation_title": (
                db.query(Conversation).filter(Conversation.id == s.conversation_id).first().title
                if db.query(Conversation).filter(Conversation.id == s.conversation_id).first()
                else "Unknown"
            ),
